...
public class ClusterSimilarityService {
    ...
    public SimilarityMatrix calculateClusterSimilarity(ModelData modelData) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            for (int j = 0; j < k; j++) {
                if (i == j) {
                    similarityMatrix[i][j] = 1.0; // Диагональ
                } else {
    ...
    /**
     * Упрощенный метод - использует готовые методы из qualityService
     */
    private double calculateSilhouetteBasedSimilarity(double[][] features, int[] labels,
                                                      int clusterA, int clusterB) {
        ...
        for (int i = 0; i < features.length; i++) {
            if (labels[i] == clusterA) {
                double a = qualityService.calculateAverageDistanceToOwnCluster(features, labels, i);
